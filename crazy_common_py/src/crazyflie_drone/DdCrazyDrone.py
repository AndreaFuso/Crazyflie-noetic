...
class CrazyDrone:
    ...
    def __init__(self, name, URI, initialPosition):
        ...
        # Ranges output logger configuration:
        self.__range_output_config = LogConfig(name='range_output_conf', period_in_ms=10)
        self.__range_output_config.add_variable('range.front', 'uint16_t') #[mm]
        self.__range_output_config.add_variable('range.back', 'uint16_t')
        self.__range_output_config.add_variable('range.left', 'uint16_t')
        self.__range_output_config.add_variable('range.right', 'uint16_t')
        self.__range_output_config.add_variable('range.up', 'uint16_t')
        ...
        # Laser reading:
        self.__laser_reading = Float64MultiArray()
        ...
        self.cmd_vel_sub = rospy.Subscriber('/' + self.cfName + '/cmd_vel', Position, self.__cmd_vel_callback)
        self.cmd_velocity = Position()
        ...
        # Publisher to publish the laser readings:
        self.__laser_reading_pub = rospy.Publisher("/cf1/laser_reading_tot", Float64MultiArray, queue_size=1)
        ...
        # Laser reading logger
        self.__range_output_logger = SyncLogger(self.__scf, self.__range_output_config)
        self.__range_output_logger.connect()
    ...
    def __pace_100Hz_cb(self, msg):
        if self.__initialOperationsEnded:
            ...
            range_output_data = self.__range_output_logger.next()
            ...
            # Extracting laser range 
            self.__laser_reading.data = [(range_output_data[1]['range.front'])*0.001,
                                         range_output_data[1]['range.back']*0.001,
                                         range_output_data[1]['range.left']*0.001,
                                         range_output_data[1]['range.right']*0.001,
                                         range_output_data[1]['range.up']*0.001,
                                         0.5]
            # self.__laser_reading.data[0] = range_output_data[1]['range.front']*0.001
            # self.__laser_reading.data[1] = range_output_data[1]['range.back']*0.001
            # self.__laser_reading.data[2] = range_output_data[1]['range.left']*0.001
            # self.__laser_reading.data[3] = range_output_data[1]['range.right']*0.001
            # self.__laser_reading.data[4] = range_output_data[1]['range.up']*0.001
            # self.__laser_reading.data[5] = 0.5 
            #do not consider the bottom laser, but it's defined in other algorithm,so just put a value,not use
            ...
            self.__desired_state_pub.publish(self.__desired_state)
             ...
             # Publishing laser range data:
            self.__laser_reading_pub.publish(self.__laser_reading)
...
## This is customized callback for room mapping implementation

    def __cmd_vel_callback(self, msg):

        self.goal.action_goal.goal.destination_info.desired_velocity.x = msg.desired_velocity.x
        self.goal.action_goal.goal.destination_info.desired_velocity.x = msg.desired_velocity.x
        self.goal.action_goal.goal.destination_info.desired_velocity.x = msg.desired_velocity.x
        self.goal.action_goal.goal.destination_info.desired_yaw_rate = msg.desired_yaw_rate

        # Sends the goal to the action server.
        self.__rel_vel_move_client.send_goal(self.goal.action_goal.goal)
