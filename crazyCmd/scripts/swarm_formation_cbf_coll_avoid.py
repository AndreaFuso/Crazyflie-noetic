...
def nlp_solver_2d(N_cf, P_N, P_0, N, x_opt, v_opt, 
                  ...
                  w_final, w_vel, N_neigh, list_neigh_formation):
    ...
    # 3
    for ii in range(N_cf-1):
        for kk in list_neigh_formation[ii]:
            # Separation cost
            d_ref_sep = d_ref
            L += w_sep*((x[ii*2]-x[kk*2])**2 \
                + (x[ii*2+1]-x[kk*2+1])**2\
    ...
    # Final Position cost
    L += w_final*((x[0] - x_des)**2 + (x[1] - y_des)**2)
...
if __name__ == '__main__':
    ...
    d_ref = 0.6 # + 0.075*N_cf # + 0.05*number_of_cfs # 0.15*number_of_cfs # reference distance between agents
    # d_ref = 0.6
    ...
    # List of neighbours for the drones (10 drones - from 0 to 9)
    ...
    list_neigh_formation = []
    list_0 = [1, 2, 3, 4, 5, 6]
    list_neigh_formation.append(list_0)
    list_1 = [2, 6] #, 7, 8, 9
    list_neigh_formation.append(list_1)
    list_2 = [3]
    list_neigh_formation.append(list_2)
    list_3 = [4]
    list_neigh_formation.append(list_3)
    list_4 = [5]
    list_neigh_formation.append(list_4)
    list_5 = [6]
    list_neigh_formation.append(list_5)

    # list_6 = [7]
    # list_neigh_formation.append(list_6)
    # list_7 = [8]
    # list_neigh_formation.append(list_7)
    # list_8 = [9]
    # list_neigh_formation.append(list_8)

    print(list_neigh_formation)
    ...
    while not rospy.is_shutdown():
        ...
        if sub_mpc_flag.data == 0:
            ...
            mpc_velocity, x_opt, v_opt = nlp_solver_2d(N_cf, P_N, P_0, 
                                                       ...
                                                       d_ref, d_neigh, v_ref,
                                                       w_sep, w_final, w_vel, N_neigh, list_neigh_formation)
            ...
            for ii in range(N_cf):
                ...
                # Appending the other drones as obstacles
                for jj in range(N_cf):
                    if ii != jj:
