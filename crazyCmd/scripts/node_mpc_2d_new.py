...
def nlp_solver_2d(mpc_target, actual_state, x_obs, y_obs, r_obs, T_mpc, N_mpc, r_drone, r_safety):
    ...
    L = v1**2 + v2**2 + 0.8*(x1-x_des)**2 + 0.8*(x2-y_des)**2
    ...
    v_ig = [(x_des-x_pos)/T_mpc, (y_des-y_pos)/T_mpc]
    ...
    for k in range(N_mpc):
        ...
        lbw += [-inf, -inf]
        ubw += [ inf,  inf]
        w0  += [X_i[0].__float__() + (k+1)/N_mpc*(P_N[0]-P_0[0]), 
                X_i[1].__float__() + (k+1)/N_mpc*(P_N[1]-P_0[1])]
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        if (mpc_target.desired_position.x == mpc_target_init.desired_position.x 
            ...
            pass
        # elif (abs(mpc_target.desired_position.x - actual_state.position.x)<0.15 and
        #       abs(mpc_target.desired_position.y - actual_state.position.y)<0.15):
        #     # Stopping the drone when it's close to target
        #     mpc_velocity.desired_velocity.x = 0.0
        #     mpc_velocity.desired_velocity.y = 0.0
        #     mpc_velocity.desired_velocity.z = 0.0
        #     mpc_velocity_pub.publish(mpc_velocity)
        #     # Switching to position PID control to reach the final target
        #     mpc_switch_pub.publish(mpc_target)

        #     # Reinitializing the mpc_target_init variable so that we can use the mpc controller
        #     # multiple times (otherwise the loop is broken before we can publish the new mpc_target)
        ...
        #     break
            ...
            mpc_velocity = nlp_solver_2d(mpc_target, actual_state, 
                                         x_obs, y_obs, r_obs, T_mpc, N_mpc, r_drone, r_safety)
            mpc_velocity_pub.publish(mpc_velocity)
